...
: boost 1.56.0
7za x -o"%cd%\tmp" "%cd%\tmp\boost_1_56_0.7z"
...
: icu 53.1
7za x -o"%cd%\tmp" "%cd%\tmp\icu4c-53_1-src.tgz"
7za x -o"%cd%\tmp" "%cd%\tmp\icu4c-53_1-src.tar"
...
: b2
pushd tmp\boost_1_56_0\tools\build
call bootstrap.bat
...
: compile
pushd tmp\boost_1_56_0
:b2 --prefix=%cd%\..\..\boost_1_56_0 --without-mpi toolset=msvc-12.0 variant=debug,release threading=multi link=static address-model=32 install
b2 --prefix=%cd%\..\..\boost_1_56_0 --without-mpi toolset=msvc-12.0 variant=debug,release threading=multi link=static runtime-link=static address-model=32 install
popd